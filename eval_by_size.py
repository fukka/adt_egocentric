...
-----
  ...
  Companion .npy files are resolved from --masks automatically:
    sam2_masks_f0.json  →  sam2_masks_f0.npy    (SAM predicted masks)
                        →  gt_seg_rot_f0.npy     (GT segmentation, saved by run_sam2.py)
...
def main():
    ...
    parser.add_argument("--rgb",        default=None,
                        help="Path to the input RGB image (e.g. real_rot90_f0.png).  "
                             ...
                             "gt_seg_rot_f<N>.npy and sam2_masks_f<N>.npy are "
                             "resolved automatically from --masks.")
    ...
    if args.rgb:
        ...
        missing = [
            (name, path) for name, path in [
                (gt_seg_stem + ".npy",                 gt_seg_path),
                (os.path.basename(sam_masks_path), sam_masks_path),
            ] if not os.path.isfile(path)
        ]
        if missing:
            print(f"\n  [overlay] Skipping — file(s) not found:")
            for name, path in missing:
                print(f"    {name}  →  {path}")
            print(f"  Run run_sam2.py first to generate these files.")
        else:
